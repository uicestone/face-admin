        ...
        .col-sm-12.mt-3.mb-3
          el-table.table-striped(:data='passRecords', border='', @row-click='showDetail', style='width: 100%')
            ...
            el-table-column(label='定金', width='70px', class-name='text-center')
              template(slot-scope='scope')
                i.nc-icon.nc-check-2.text-success(v-if='scope.row.assured')
                i.nc-icon.nc-simple-remove.text-danger(v-else='')
        ...
        .card-body.row
          ...
          .col-sm-12.mt-3.mb-3
            el-table.table-striped(:data='passRecords' border='' @row-click='showDetail' style='width: 100%')
              ...
              el-table-column(prop='child.name' label='孩子')
              el-table-column(prop='resident.name' label='居民')
              el-table-column(label='创建时间' width='115px')
                template(slot-scope='scope')
                  | {{ scope.row.createdAt | date(&apos;M/DD H:mm&apos;) }}
    ...
    passRecords: {
      ...
      variables() {
        ...
        if (this.searchQuery.resident) {
          where.resident = {
            user: { name: { _like: `%${this.searchQuery.resident}%` } }
          };
        }
        return {
          where,
